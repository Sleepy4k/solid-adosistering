...
function Dashboard() {
  ...
  function toggleTheme() {
    setStore("user", "settings", "theme", (prev) => (prev === "light" ? "dark" : "light"));
  }
...
const FancyInput = forwardRef<HTMLInputElement, { label: string }>(({ label }, ref) => {
  return (
    <label>
      {label}
      <input ref={ref} />
...
function FancyInput(props: { label: string; ref: HTMLInputElement | ((el: HTMLInputElement) => void) }) {
  return (
    <label>
      {props.label}
      <input ref={props.ref} />
