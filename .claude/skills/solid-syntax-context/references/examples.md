...
export function ThemeProvider(props: ParentProps<{ theme: keyof typeof themes }>) {
...
function ThemedCard() {
  ...
  return <div style={{ background: theme.bg, color: theme.fg, padding: "16px" }}>Current theme: {theme.name}</div>;
}
...
export function AppProviders(props: ParentProps) {
  return <ComposeProviders providers={[AuthProvider, CartProvider, LocaleProvider]}>{props.children}</ComposeProviders>;
}
```
