...
function FilteredList() {
  ...
  const filtered = createMemo(() => items().filter((item) => item.toLowerCase().includes(query().toLowerCase())));

  return (
    <div>
      <input value={query()} onInput={(e) => setQuery(e.target.value)} placeholder="Filter..." />
      <p>Showing {filtered().length} items</p>
      <For each={filtered()}>{(item) => <div>{item}</div>}</For>
    </div>
  );
}
```
...
const position = createMemo(() => rawPosition(), undefined, {
  equals: (prev, next) => Math.abs(prev.x - next.x) < 0.001 && Math.abs(prev.y - next.y) < 0.001,
});
```
...
function SearchResults() {
  ...
  return (
    <div>
      ...
      <For each={results()}>{(item) => <div>{item.name}</div>}</For>
    </div>
  );
}
```
