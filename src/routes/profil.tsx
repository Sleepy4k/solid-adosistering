...
import { getMyProfile, updateMyProfile, changeMyPassword, type MyProfile } from "~/server/actions";
...
function EditProfileModal(props: { profile: MyProfile; onClose: () => void }) {
  ...
  const save = async (e: SubmitEvent) => {
    ...
    setLoading(true);
    try {
      await updateMyProfile({ name: name(), whatsapp: whatsapp(), gender: gender(), address: address(), country: country(), province: province(), city: city(), postalCode: postalCode() });
  ...
  return (
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && props.onClose()}>
      <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl" style={{ animation: "modal-in 200ms ease" }}>
        ...
        <form onSubmit={save} class="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input value={postalCode()} onInput={(e) => setPostalCode(e.currentTarget.value)} class="input-base w-full" />
          </div>
          <div class="flex justify-end gap-3 sm:col-span-2">
            <button type="button" class="btn-outline" onClick={props.onClose}>Batal</button>
            <button type="submit" class="btn-primary" disabled={loading()}>{loading() ? "Menyimpan…" : "Simpan"}</button>
          </div>
        </form>
      </div>
    </div>
  ...
  return (
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && props.onClose()}>
      <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl" style={{ animation: "modal-in 200ms ease" }}>
        ...
        <Show when={error()}><div class="mb-3 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{error()}</div></Show>
        <form onSubmit={save} class="flex flex-col gap-4">
          <div><label class="mb-1 block text-xs font-medium text-slate-600">Password Saat Ini</label><input type="password" value={current()} onInput={(e) => setCurrent(e.currentTarget.value)} class="input-base w-full" required /></div>
          <div><label class="mb-1 block text-xs font-medium text-slate-600">Password Baru</label><input type="password" value={newPw()} onInput={(e) => setNewPw(e.currentTarget.value)} class="input-base w-full" required /></div>
          <div><label class="mb-1 block text-xs font-medium text-slate-600">Konfirmasi Password</label><input type="password" value={confirmPw()} onInput={(e) => setConfirmPw(e.currentTarget.value)} class="input-base w-full" required /></div>
...
export default function Profil() {
  ...
  return (
    <>
      <Title>Profil | Adosistering</Title>
      <Meta name="description" content="Informasi profil pengguna dan pengaturan keamanan akun." />

      <div class="flex flex-col gap-5">
        <h1 class="text-2xl font-bold text-slate-900">Profil</h1>

        <Suspense fallback={<div class="flex flex-col gap-4"><SkCard /><SkCard /><SkCard /></div>}>
          <Show when={profile()}>
            {(p) => (
              <>
                {/* Profil Saya */}
                ...
                <SectionCard title="Keamanan">
                  <div class="mb-4 flex justify-end">
                    <button type="button" class="btn-outline text-sm" onClick={() => setEditPw(true)}>Ubah Kata Sandi</button>
                  </div>
                  <div class="grid gap-4 sm:grid-cols-2">
