...
export function AppProvider(props: ParentProps) {
  ...
  const actions = {
    ...
    addNotification: (message: string) =>
      setState("notifications", (prev) => [...prev, { id: crypto.randomUUID(), message }]),
...
const subtotal = createMemo(() => items().reduce((sum, item) => sum + item.price * item.qty, 0));
...
function UserForm() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <select value={form.role} onChange={(e) => setForm("role", e.currentTarget.value as FormData["role"])}>
...
function Button(props: {
  ...
  return (
    <button {...others} class={`btn btn-${local.variant} btn-${local.size} ${others.class ?? ""}`}>
      {local.children}
    </button>
  );
}
