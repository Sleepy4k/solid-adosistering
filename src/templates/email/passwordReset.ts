...
export function passwordResetTemplate(resetUrl: string, config?: Partial<EmailBrandConfig>) {
  const primaryColor = config?.primaryColor ?? "#186D3C";
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 16px;font-size:12px;color:${primaryColor};word-break:break-all">${resetUrl}</p>
