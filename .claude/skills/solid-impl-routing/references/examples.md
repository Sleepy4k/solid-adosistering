...
function ProductList() {
  ...
  return (
    <div>
      <div class="filters">
        <select value={search.sort ?? "name"} onChange={(e) => setSearch({ sort: e.currentTarget.value, page: "1" })}>
          <option value="name">Name</option>
          <option value="price">Price</option>
        </select>

        <button onClick={() => setSearch({ category: undefined })}>Clear Category</button>
      </div>

      <div class="pagination">
        <button disabled={currentPage() <= 1} onClick={() => setSearch({ page: String(currentPage() - 1) })}>
          Previous
        </button>
        <span>Page {currentPage()}</span>
        <button onClick={() => setSearch({ page: String(currentPage() + 1) })}>Next</button>
      </div>
    </div>
  );
}
...
function Navigation() {
  ...
  return (
    <nav>
      <A href="/dashboard" onMouseEnter={() => preload("/dashboard")} onFocus={() => preload("/dashboard")}>
        Dashboard
      </A>
      <A href="/analytics" onMouseEnter={() => preload("/analytics")} onFocus={() => preload("/analytics")}>
        Analytics
      </A>
    </nav>
  );
