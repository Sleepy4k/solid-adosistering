...
const routes: RouteDefinition[] = [
  ...
  { path: "/about", component: lazy(() => import("./pages/About")) },
  {
    ...
    children: [{ path: "/:id", component: lazy(() => import("./pages/UserDetail")) }],
  },
];
...
function ProductList() {
  ...
  return (
    <div>
      ...
      <button onClick={() => setSearch({ page: String(Number(search.page ?? 1) + 1) })}>Next Page</button>
    </div>
  );
}
```
...
```tsx
<Route path="/users/:id" component={UserPage} matchFilters={{ id: /^\d+$/ }} />
```
