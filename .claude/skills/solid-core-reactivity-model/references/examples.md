...
```tsx
import { createSignal, createEffect, createRoot, getOwner, runWithOwner, onCleanup } from "solid-js";
...
function AsyncDataProcessor() {
  ...
  onMount(async () => {
    ...
    runWithOwner(owner!, () => {
      createEffect(() => {
        const filtered = data.filter((item: any) => matchesFilter(item, filter()));
        updateUI(filtered);
      });
    });
  });
...
function WebSocketComponent() {
  ...
  return (
    <div>
      ...
      <button onClick={() => setUrl("wss://example.com/other")}>Switch Feed</button>
    </div>
  );
}
```
