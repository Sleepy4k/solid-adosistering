...
function useAction<T extends Array<any>, U>(action: Action<T, U>): (...args: T) => Promise<U>;
```
...
function useSubmission<T extends Array<any>, U>(action: Action<T, U>, filter?: (input: T) => boolean): Submission<T, U>;
...
function reload(options?: { revalidate?: string | string[] }): Response;
```
...
function revalidate(key?: string | string[], force?: boolean): Promise<void>;
```
