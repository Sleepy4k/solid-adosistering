...
export function CounterProvider(props: ParentProps) {
  ...
  return <CounterContext.Provider value={[count, actions]}>{props.children}</CounterContext.Provider>;
}
...
export function ThemeProvider(props: ParentProps) {
  ...
  return <ThemeContext.Provider value={{ theme, resolvedTheme, setTheme }}>{props.children}</ThemeContext.Provider>;
}
...
function ThemeToggle() {
  ...
  return (
    <button onClick={() => setTheme(resolvedTheme() === "light" ? "dark" : "light")}>Current: {resolvedTheme()}</button>
  );
}
```
...
export function AppProvider(props: ParentProps) {
  ...
  const actions: AppActions = {
    addTodo: (title: string) => {
      setState("todos", (prev) => [...prev, { id: crypto.randomUUID(), title, done: false }]);
    },
...
function ProductList() {
  ...
  const filteredBySearch = createMemo(() => {
    ...
    return filteredByStock().filter((p) => p.name.toLowerCase().includes(query));
  });

  const totalValue = createMemo(() => filteredBySearch().reduce((sum, p) => sum + p.price, 0));

  return (
    <div>
      <input placeholder="Search..." value={search()} onInput={(e) => setSearch(e.currentTarget.value)} />
      <label>
        <input
          type="checkbox"
      ...
      <p>
        Showing {filteredBySearch().length} items (total: ${totalValue()})
      </p>
...
function RegistrationWizard() {
  ...
  const canProceed = (): boolean => {
    switch (step()) {
      case 1:
        return form.step1.name.length > 0 && form.step1.email.includes("@");
      case 2:
        return form.step2.company.length > 0;
      case 3:
        return form.step3.agree;
      default:
        return false;
    }
  };
...
function Clock() {
  ...
  const seconds = from(interval(1000).pipe(map(() => new Date().toLocaleTimeString())));
...
function NotificationList() {
  ...
  return <For each={state.notifications}>{(n) => <div class={n.read ? "read" : "unread"}>{n.message}</div>}</For>;
}
```
